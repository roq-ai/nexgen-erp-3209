...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Order #12345 has been placed by customer John Doe.create': {
    roles: ['business-owner', 'purchasing-manager'],
    key: 'order_placed',
    tenantPath: [],
    userPath: [],
  },
  'Purchase order #67890 has been created for supplier XYZ.create': {
    roles: ['business-owner', 'inventory-manager'],
    key: 'purchase_order_created',
    tenantPath: [],
    userPath: [],
  },
  'Inventory for product ABC has reached a critical level.update': {
    roles: ['inventory-manager', 'purchasing-manager'],
...
const tenantRoles: string[] = ['business-owner', 'purchasing-manager', 'inventory-manager'];
